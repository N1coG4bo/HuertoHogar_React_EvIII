...
function PixelBlastBackground({
  ...
  useEffect(() => {
    ...
    const trackedKeys = ['antialias', 'liquid', 'noiseAmount'];
    const trackedParams = { antialias, liquid, noiseAmount };
    let shouldRecreate = false;
    if (!instanceRef.current) shouldRecreate = true;
    else if (lastParamsRef.current) {
      for (const key of trackedKeys) {
        if (lastParamsRef.current[key] !== trackedParams[key]) {
          shouldRecreate = true;
          break;
        }
      }
    ...
    if (shouldRecreate) {
      if (instanceRef.current) {
        ...
        existing.quad?.geometry?.dispose();
        existing.material?.dispose();
        existing.composer?.dispose();
        existing.renderer?.dispose();
        if (existing.renderer?.domElement?.parentElement === container) {
          container.removeChild(existing.renderer.domElement);
        }
      ...
      const resize = () => {
        ...
        if (instanceRef.current?.composer) {
          instanceRef.current.composer.setSize(renderer.domElement.width, renderer.domElement.height);
        }
      ...
      const getPointer = (event) => {
        ...
        const mx = renderer.domElement.width / rect.width;
        const my = renderer.domElement.height / rect.height;
        const fx = (event.clientX - rect.left) * mx;
        const fy = (rect.height - (event.clientY - rect.top)) * my;
        return { fx, fy, w: renderer.domElement.width, h: renderer.domElement.height };
      };

      const onClick = (event) => {
        const { fx, fy } = getPointer(event);
        const idx = instanceRef.current?.clickIx ?? 0;
        uniforms.uClickPos.value[idx].set(fx, fy);
        uniforms.uClickTimes.value[idx] = uniforms.uTime.value;
        if (instanceRef.current) {
          instanceRef.current.clickIx = (idx + 1) % MAX_CLICKS;
        }
      };

      const onMove = (event) => {
        if (!touch) return;
        const { fx, fy, w, h } = getPointer(event);
      ...
      instance.uniforms.uShapeType.value = SHAPE_MAP[variant] ?? 0;
      instance.uniforms.uPixelSize.value = pixelSize * instance.renderer.getPixelRatio();
      ...
      if (transparent) instance.renderer.setClearAlpha(0);
      else instance.renderer.setClearColor(0, 1);
      if (instance.liquidEffect) {
        instance.liquidEffect.uniforms.get('uStrength').value = liquidStrength;
        instance.liquidEffect.uniforms.get('uFreq').value = liquidWobbleSpeed;
      }
      if (instance.touch) {
        instance.touch.radiusScale = liquidRadius;
      }
    }

    lastParamsRef.current = trackedParams;
    return () => {
      if (!instanceRef.current || shouldRecreate) return;
      const existing = instanceRef.current;
      existing.resizeObserver?.disconnect();
      cancelAnimationFrame(existing.raf);
      existing.quad?.geometry?.dispose();
